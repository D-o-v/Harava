...
import { Save, Camera, Loader2, ShieldCheck, KeyRound, Mail, Smartphone } from "lucide-react";
  ...
  return (
    <div className="min-h-screen">
            ...
            <div className="flex items-center justify-between">
              ...
              {user?.mfaEnabled ? (
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="ghost" size="sm" onClick={async () => { try { await enableEmail.mutate(); toast("Email MFA enabled", "success"); await refreshMe(); } catch (e) { toast(e instanceof Error ? e.message : "Failed", "error"); } }}><Mail className="w-3 h-3" /> Email</Button>
                  <Button variant="ghost" size="sm" onClick={async () => { try { await enableSms.mutate(phone); toast("SMS MFA enabled", "success"); await refreshMe(); } catch (e) { toast(e instanceof Error ? e.message : "Failed", "error"); } }}><Smartphone className="w-3 h-3" /> SMS</Button>
                </div>
              )}
            </div>
          </CardContent>
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button variant="primary" onClick={async () => {
              try { await changePw.mutate(currentPw, newPw); toast("Password changed", "success"); setPwModal(false); setCurrentPw(""); setNewPw(""); }
              catch (e) { toast(e instanceof Error ? e.message : "Failed", "error"); }
            }} disabled={!currentPw || !newPw || changePw.loading}>Save</Button>
          </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={totpModal} onClose={() => setTotpModal(false)} title="Set up authenticator">
        <div className="space-y-3">
          {totpSecret?.qrCode ? (
            <img src={totpSecret.qrCode.startsWith("data:") ? totpSecret.qrCode : `data:image/png;base64,${totpSecret.qrCode}`} alt="QR" className="mx-auto w-48 h-48" />
